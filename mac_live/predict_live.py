...
def cohort_due(conn) -> bool:
    ...
    sessions = conn.execute("SELECT COUNT(DISTINCT date) FROM daily_prices WHERE date > ?",
                            (last,)).fetchone()[0]
    return sessions >= config.EXEC_HOLD_DAYS
...
def main() -> None:
    ...
    if not cohort_due(conn):
        print("\n  Cohort not due yet (one book per "
              f"{config.EXEC_HOLD_DAYS} trading sessions) - scoring only today.")
        conn.close()
        return
...
def expire_stale_pending(conn) -> int:
    ...
    for tid, pick_date in conn.execute(
        ...
        if sessions > config.EXEC_HOLD_DAYS:
...
def score_paper_trades(conn) -> int:
    ...
    for tid, pick_date, ticker, entry_close, entry_open in conn.execute(
        ...
        fwd = conn.execute(
            ...
            (ticker, pick_date, config.EXEC_HOLD_DAYS + 1)).fetchall()
        window = fwd[:config.EXEC_HOLD_DAYS] if legacy else fwd       # F1 window includes the entry session
        need = config.EXEC_HOLD_DAYS if legacy else config.EXEC_HOLD_DAYS + 1
        if len(window) < need:
