...
The whole system in one resilient command:
  ...
  3. score any paper trades that have matured (10 trading days elapsed)
...
def main() -> None:
    ...
    status = {"data": "skip", "picks": "skip", "score": "skip"}
    ...
    try:
        ...
        latest = _latest_data_date()                         # freshness guard
        gap = (date.today() - date.fromisoformat(latest)).days if latest else 999
        if gap > 4:
            print(f"\n  [WARN] latest price data is {latest} ({gap}d old) - picks may be stale.")

        try:                                                 # 2. generate + log picks
            predict_live.main()
            status["picks"] = "ok"
        except Exception:
            status["picks"] = "FAILED"
        ...
        try:                                                 # 3. score matured paper trades
            ...
            print(f"\n  Paper-trade record: {rec['win']}W / {rec['loss']}L (win {wr}), "
                  f"{rec['open']} open   (+{n} scored today)")
            status["score"] = "ok"
        except Exception:
            status["score"] = "FAILED"
            traceback.print_exc()

        print(f"\n  STATUS: data={status['data']}  picks={status['picks']}  score={status['score']}")
    finally:
